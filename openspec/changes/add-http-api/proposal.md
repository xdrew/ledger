...
- **OpenAPI 3.1 generated by a custom reflection-based generator** (the same approach as the
...
- **New code:** `App\Api\` (invokable `*Action` controllers, request/response DTOs + validation,
  an auth listener, an idempotency listener, an RFC 9457 exception listener); a custom
