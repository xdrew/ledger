...
> nyholm/psr7, symfony/psr-http-message-bridge), symfony/validator. OpenAPI is generated by a
> custom reflection generator (no nelmio/swagger-php) and verified without an OpenAPI-validator
> dependency. The production image and Helm are deferred to add-deployment.
...
- [ ] 4.3 Generator test: the document is well-formed OpenAPI 3.1 with the expected paths, operations, and component schemas. (Per-endpoint response shapes are covered by the functional tests in §5.)
